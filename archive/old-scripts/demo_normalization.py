...
def simulate_normalization(original, proposed_normalized, extractor):
    ...
    if verification['preserved']:
        print("[SUCCESS] QUALIFIER PRESERVATION: PASSED")
        print("   All qualifiers maintained in normalized version")
        confidence = 0.95
    else:
        print("[FAIL] QUALIFIER PRESERVATION: FAILED")
...
def main():
    ...
    print()
    print("  [OK] Modal qualifiers (can, may, might, etc.) are NEVER lost")
    print("  [OK] Frequency qualifiers (always, never, often, etc.) are preserved")
    print("  [OK] Quantity qualifiers (all, some, most, etc.) remain intact")
    print()
    print("This prevents catastrophic meaning changes like:")
    print("  [X] 'X may cause Y' -> 'X causes Y'")
    print("  [X] 'Some evidence suggests' -> 'Evidence shows'")
    print("  [X] 'Often associated with' -> 'Associated with'")
    print()
    print("=" * 80)
