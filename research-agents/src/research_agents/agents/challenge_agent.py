...
class ChallengeAgent(BaseAgent):
    ...
    def __init__(
        ...
        if config is None:
            config = AgentConfig(agent_type="challenge")

        super().__init__(
            InvestigationFramework.CHALLENGE,
            db,
            ai,
            config,
        )
    ...
    async def investigate(self, claim: Claim) -> Finding:
        ...
        Returns:
            Finding with challenging evidence
        """
        logger.info(f"Finding challenges for: {claim.text}")

        # Search for challenging evidence
        evidence_list = await self._search_challenging_evidence(claim.text)

        # Calculate confidence impact (negative for challenging evidence)
...
Return ONLY the JSON array, no other text."""
        ...
        try:
            ...
            for item in evidence_data:
                evidence = Evidence(
                    ...
                    supports_claim=False,  # Challenge agent finds contradicting evidence
                    publication_year=item.get('publication_year'),
                    created_at=datetime.utcnow(),
                )
                evidence_list.append(evidence)

            logger.info(f"Found {len(evidence_list)} challenging sources")
    ...
    def _calculate_confidence_impact(self, evidence_list: List[Evidence]) -> float:
        ...
        # Negative impact since this is challenging evidence
        # Scale: -0.3 for weak challenge, -0.7 for strong challenge
        impact = -1 * avg_relevance * 0.5  # Negative and capped at -0.5
